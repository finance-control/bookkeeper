...
internal class ReportTest {
    ...
    fun `plus when 2 reports contains different categories then both categories are in the result report`() {
        val report1 = Report(
            byCategory = mapOf(4.asId<SpendingCategory>() to listOf(Money(Currency.EUR, 5, 0))),
            total = listOf(Money(Currency.EUR, 5, 0))
        )

        val report2 = Report(
            byCategory = mapOf(5.asId<SpendingCategory>() to listOf(Money(Currency.EUR, 7, 0))),
            total = listOf(Money(Currency.EUR, 7, 0))
        )

        val expected = Report(
            byCategory = mapOf(
                5.asId<SpendingCategory>() to listOf(Money(Currency.EUR, 7, 0)),
                4.asId<SpendingCategory>() to listOf(Money(Currency.EUR, 5, 0))
            ),
    ...
    fun `plus when 2 reports contains different categories then summarize that moneys`() {
        val report1 = Report(
            byCategory = mapOf(5.asId<SpendingCategory>() to listOf(Money(Currency.USD, 5, 0))),
        ...
        val expected = Report(
            byCategory = mapOf(
                5.asId<SpendingCategory>() to listOf(Money(Currency.USD, 5, 0), Money(Currency.EUR, 7, 0)),
            ),
