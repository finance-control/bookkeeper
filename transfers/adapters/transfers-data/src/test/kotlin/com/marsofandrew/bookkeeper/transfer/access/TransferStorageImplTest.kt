...
internal class TransferStorageImplTest {
    ...
    private val transferById: MutableMap<NumericId<Transfer>, Transfer> = mutableMapOf()
    ...
    fun `create when correct values are provided then creates transfer with unique id`() {
        val transfer = transfer(NumericId.unidentified(), userId)
    ...
    fun `delete does nothing when ids are not exists`() {
        transferstorageImpl.delete(setOf(NumericId(1)))
    ...
    fun `findAllByUserId when transfers for user exist returns those transfers`() {
        val transfers = listOf(
            transfer(NumericId.unidentified(), userId),
            transfer(NumericId.unidentified(), userId) {
                received = AccountMoney.create(PositiveMoney(Currency.EUR, 100))
            },
    ...
    fun `findAllByUserIdBetween when transfers within interval are absent returns empty list`() {
        val now = LocalDate.now()
        transferstorageImpl.create(transfer(NumericId.unidentified(), userId) { date = now.plusDays(1) })
    ...
    fun `findAllByUserIdBetween when transfers for user within interval exist returns those transfers`() {
        ...
        val transfers = listOf(
            transfer(NumericId.unidentified(), userId) { date = now },
            transfer(NumericId.unidentified(), userId) { date = startDate },
            transfer(NumericId.unidentified(), userId) { date = startDate.minusDays(1) },
            transfer(NumericId.unidentified(), userId) { date = now.plusDays(1) },
            transfer(NumericId.unidentified(), userId) { date = now },
        ).map(transferstorageImpl::create)
